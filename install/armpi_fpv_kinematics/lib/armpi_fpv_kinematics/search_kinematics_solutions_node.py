...
# @data:2023/03/20
# @author:aiden
# 实时获取角度反馈，根据当前位置和(real-time angle feedback, based on the current position)
# 目标位置的最小差值来获取最优解(the minimum difference between the current position and the target position to obtain the optimal solution)
import rospy
import numpy as np
...
fk = ForwardKinematics(debug=False)  # 不开启打印(do not enable printing)
...
class SearchKinematicsSolutionsNode:
    ...
    def set_joint_value_target(self, msg):
        # 正运动学解(forward kinematic solution)
    ...
    def get_current_pose(self, msg):
        # 获取机械臂当前位置(get robotic arm current position)
    ...
    def get_servo_position(self, msg):
        # 获取舵机当前角度(get servo current angle)
        servo_positions = []
    ...
    def set_pose_target(self, msg):
        # 逆运动学解，获取最优解(所有电机转动最小)(Inverse kinematics solution, obtaining the optimal solution (minimizing rotation for all motors))
