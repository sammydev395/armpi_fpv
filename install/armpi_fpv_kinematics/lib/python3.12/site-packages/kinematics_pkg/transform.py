...
# 连杆长度(m)(length of the link (m))
# 底座的高度，这里把第一个坐标系和第二个坐标的原点重合到一起了(height of the base, where the origin of the first and second coordinate systems is aligned)
base_link = 0.064605

link1 = 0.10048
link2 = 0.094714

# 计算tool_link时取值为link3 + tool_link，因为把末端的坐标系原点和前一个重合到一起了(when calculating tool_link, the value is taken as link3 + tool_link because the origin of the coordinate system at the end is aligned with the previous one)
# 这里的tool_link指实际上的夹持器长度(here, tool_link refers to the actual length of the gripper)
link3 = 0.05071
tool_link = 0.1126

# 各关节角度限制，取决于是否碰撞以及舵机的转动范围(joint angle limits depend on collision and the range of motion of the servo motors)
# 多加0.2为了防止计算时数值的不稳定，会比设定值大一点点(add more 0.2 to prevent numerical instability during calculation, resulting in slightly larger values than the set ones)
joint1 = [-120.2, 120.2]
joint2 = [-180.2, 0.2]
joint3 = [-120.2, 120.2]
joint4 = [-200.2, 20.2]
...
#         舵机脉宽范围，中位值，对应的角度范围，中位值(servo pulse width range, median value, corresponding angle range, and median value)
...
# 判断是否为旋转矩阵(check if it is a rotation matrix)
