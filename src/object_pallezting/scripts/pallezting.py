...
# Date:2024/02/05
# Author:Aiden
# 第13章 ArmPi FPV创意玩法课程\1.AI视觉创意玩法\第5课 智能码垛（13. creative AI application course\1. AI vision creative program\lesson 5 intelligent stacking)
import cv2
import copy
import math
import queue
...
################################################
# 放置坐标x, y, z(m)(placement coordinate x, y, z(m))
...
tag_x_dis = 500
tag_y_dis = 0
tag_x_pid = pid.PID(P=0.01, I=0.001, D=0)#pid初始化(pid initialization)
...
def pick(grasps, have_adjust=False):
    ...
    if target1[1] and target2[1] and target3[1] and target4[1]:
        if not have_adjust:
            ...
            if state == 'color':
                # 第四步：微调整位置(step four: slightly adjust the position)
                if not adjust:
                    adjust = True
                    return True
            else:
...
def place(places):
    ...
    if target1[1] and target2[1] and target3[1] and target4[1]:
        ...
        servo_data = target1[1]
        bus_servo_control.set_servos(joints_pub, 1, ((1, places.pre_grasp_posture), (2, int(F*rotation.y)), (3, 150), (4, 825), (5, 625), (6, servo_data[0])))
        rospy.sleep(1)
        if not running:
...
def color_sort(img, target):
    ...
    frame_lab = cv2.cvtColor(frame_resize, cv2.COLOR_BGR2LAB)  # 将图像转换到LAB空间(convert image to the LAB space)
    ...
    for i in color_range:
        if i in target:
            if i in detect_color:
                ...
                areaMaxContour, area_max = common.get_area_max_contour(contours)  # 找出最大轮廓(the maximal contour is found)
                if areaMaxContour is not None:
                    if area_max > max_area and area_max > 100:#找最大面积(find the largest contour)
                        max_area = area_max
                        color_area_max = i
                        areaMaxContour_max = areaMaxContour
    if max_area > 100:  # 有找到最大面积(the largest contour is found)
