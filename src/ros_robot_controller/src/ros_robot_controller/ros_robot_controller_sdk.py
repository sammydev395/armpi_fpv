...
class PacketFunction(enum.IntEnum):
    ...
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servo control, the board is numbered 1-4 from inside to outside)
    ...
    PACKET_FUNC_SBUS = 9  # 航模遥控获取(air-craft remote control obtain)
    PACKET_FUNC_OLED = 10 # OLED 显示内容设置(display content settings)
    PACKET_FUNC_RGB = 11 # 设置RGB颜色(set RGB color)
    PACKET_FUNC_NONE = 12

class PacketReportKeyEvents(enum.IntEnum):
    # 按键的不同状态(the different state of the button)
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(sub command 0x01 set SSID, the second byte is the string length, this length includes '\0' string termination character)
