...
# Date:2024/02/05
# Author:Aiden
# 第13章 ArmPi FPV创意玩法课程\2.智慧仓储课程\第5课 智能转仓（13. creative AI application course\2. intelligent warehouse course\Lesson 5 intelligent warehouse transfer)
import cv2
import rospy
import threading
import numpy as np
...
from warehouse.srv import SetExchangeTarget
...
def pick(grasps):
    ...
    if target1[1] and target2[1] and target3[1] and target4[1]:
        # 第一步：云台转到朝向目标方向，夹持器打开(step one: pan-tilt rotate to the target direction, open gripper)
...
def place(places):
    ...
    if target1[1] and target2[1] and target3[1] and target4[1]:
        ...
        servo_data = target1[1]
        if target_data[0][0] != target_data[1][0]:
            bus_servo_control.set_servos(joints_pub, 2, ((1, places.pre_grasp_posture), (2, 500), (3, 80), (4, 825), (5, 625), (6, servo_data[0])))
            rospy.sleep(2)
        rospy.sleep(0.5)
        if not running:
...
##########################################
## 货架每层位置x, y, z(m)(position of each shelf layer x, y, z(m))
...
# 每层放置时的俯仰角(pitch angle when placing on each layer)
roll_dict = {'R1': 50,
             'R2': 60,
             'R3': 90,
             'L1': 50,
...
# 将ros发布的图像转化成opencv能够处理的格式，并且将处理后的图像发布出去(convert the image published by ROS into a format that OpenCV can handle, and then publish the processed image)
def image_callback(ros_image):  
    global lock
    
    image = np.ndarray(shape=(ros_image.height, ros_image.width, 3), dtype=np.uint8,
                       buffer=ros_image.data)  # 将自定义图像消息转化为图像(convert the custom image information to image)
...
if __name__ == '__main__':
    ...
    rospy.init_node('exchange', log_level=rospy.INFO)
    ...
    rospy.sleep(0.2) # pub之后必须延时才能生效(there must be a delay after publishing for the changes to take effect)
