...
# Date:2024/02/05
# Author:Aiden
# 第13章 ArmPi FPV创意玩法课程\2.智慧仓储课程\第3课 智能入库（13. creative AI application course\1. AI vision creative program\2. intelligent warehousing course\Lesson 3 intelligent warehousing)
import cv2
import math
import rospy
import queue
...
def pick(grasps, have_adjust=False):
    ...
    if target1[1] and target2[1] and target3[1] and target4[1]:
        if not have_adjust:
            ...
            # 第七步：抬升物体(step seven: lift objects)
            if grasps.up != 0:
                target3 = set_pose_target((position.y, position.x, position.z + grasps.up), rotation.r, [-90, 90], 0)
...
def place(places):
    ...
    if target1[1] and target2[1] and target3[1] and target4[1]:
        ...
        # 第三步：移到目标点(step three: move to the target position)
        target2 = set_pose_target((position.y, position.x, position.z), rotation.r, [-90, 90], 0)
        servo_data = target2[1]
        bus_servo_control.set_servos(joints_pub, 0.5, ((3, servo_data[3]), (4, servo_data[2]), (5, servo_data[1]), (6, servo_data[0])))
        rospy.sleep(1)
...
###########################################
# 货架每层位置x, y, z(m)(each shelf layer position x, y, z(m))
...
# 每层放置时的俯仰角(the pitch angle when placing on each layer)
roll_dict = {'R1': 50,
             'R2': 60,
             'R3': 90,
             'L1': 50,
        ...
        if running:
            if approach:
                ...
                if not adjust and move_state == 1:
                    ...
                    # 夹取时的俯仰角(the pitch angle when grasping)
...
def color_sort(img, target):
    ...
    if max_area > 100:  # 有找到最大面积(the maximal area is found)
