...
# Date:2021/04/20
# Author:Aiden
# 第13章 ArmPi FPV创意玩法课程\2.智慧仓储课程\第4课 智能出库（13. creative AI application course\1. AI vision creative program\2. intelligent warehousing course\Lesson 4 intelligent outbound)
import cv2
import math
import rospy
import threading
...
def place(places):
    ...
    if target1[1] and target2[1] and target3[1] and target4[1]:
        ...
        # 第二步：移到接近点(step three: move to the target point)
        target1 = set_pose_target((position.y + approach.y, position.x + approach.x, position.z + approach.z), rotation.r, [-90, 90], 0)
        servo_data = target1[1]
...
# 每层货架对应放置点位置x, y, z(m)(corresponding position of each shelf layer x, y, z(m))
        ...
        if running:            
            if len(target_data) != 0:
                ...
                if running:
                    ...
                    # 夹取时的俯仰角(the pitch angle during grasping)
                    ...
                    # 夹取前后夹持器的开合(opening and closing of the rear gripper before grasping)
